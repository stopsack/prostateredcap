...
qc_criteria_pts <- function() {
  tibble::tribble(
    ...
    "Missing date of birth or diagnosis",     expr(!is.na(agedx)),
    "Metastases before diagnosis",            expr(!(dx_met_mos <= 0 & stage != "M1")),
    "Metastatic/CRPC but no associated date", expr(!(((met_date_na == TRUE & stage != "M1" & is_met == "Yes") |
                                                        (crpc_date_na == TRUE & is_crpc == "Yes")) &
                                                       lastvisit_na == TRUE)),
...
#'   Defaults to the return of \code{\link{qc_criteria_pts}}.
#'   Custom criteria can be supplied instead.
#' @param qc_crit_smp Criteria for checking the \code{smp} tibble.
#'   Defaults to the return of \code{\link{qc_criteria_smp}}.
#'   Custom criteria can be supplied instead.
...
#' * \code{qc_pts}: Tibble of sequential exclusions for \code{smp}.
#'
#' @import dplyr purrr
#' @export
#'
...
check_prostate_redcap <- function(data,
  ...
  qc_smp_data <- qc_smp %>%
    ...
    pull(included) %>%
    pluck(1) %>%
    select(-stage_for_qc, -complete_smp) %>%
    labelled::copy_labels(from = data$smp)
  ...
  if(recommended_only == TRUE) {
    ...
    qc_smp_data <- qc_smp_data %>%
      select(ptid, dmpid, hist_smp, dzextent, ext_pros, ext_lndis, ext_bone, ext_vis,
             bonevol, cntadt, tissue, smp_pros, smp_type, primmet, agesmp, dx_smp_mos, adt_smp_mos,
             smp_met_mos, smp_crpc_mos, smp_os_mos, dzvol, denovom)
  }
